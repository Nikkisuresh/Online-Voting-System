...
def voteCast(root,frame1,vote,client_socket):
    ...
    if(message=="Successful"):
        ...
        Label(frame1, text="Vote Cast Failed...", font=('Helvetica', 18, 'bold')).grid(row = 1, column = 1)
...
def votingPg(root,frame1,client_socket):
    ...
    cand_bjp = ImageTk.PhotoImage((Image.open("img/narendramodi.jpg")).resize((45,45),Image.ANTIALIAS))
    bjpcandImg = Label(frame1, image=cand_bjp).grid(row = 2,column = 2)
    ...
    cand_cong = ImageTk.PhotoImage((Image.open("img/rahulgandhi.jpg")).resize((35,48),Image.ANTIALIAS))
    congcandImg = Label(frame1, image=cand_cong).grid(row = 3,column = 2)
    ...
    cand_aap = ImageTk.PhotoImage((Image.open("img/arvindkejriwal.jpg")).resize((55,40),Image.ANTIALIAS))
    aapcandImg = Label(frame1, image=cand_aap).grid(row = 4,column = 2)

    Radiobutton(frame1, text = "Shiv Sena\n\nUddhav Thakrey", variable = vote, value = "ss", indicator = 0, height = 4, width=15, command = lambda: voteCast(root,frame1,"ss",client_socket)).grid(row = 5,column = 1)
    ...
    cand_ss = ImageTk.PhotoImage((Image.open("img/uddhav.jpg")).resize((50,45),Image.ANTIALIAS))
    sscandImg = Label(frame1, image=cand_ss).grid(row = 5,column = 2)
    ...
    cand_nota = ImageTk.PhotoImage((Image.open("img/nouser.png")).resize((45,35),Image.ANTIALIAS))
    notacandImg = Label(frame1, image=cand_nota).grid(row = 6,column = 2)
